...
export default async function HomePage({
  ...
  // 베스트 후기 (post_likes 많은 순)
  const { data: bestPosts } = await supabase
    .from('posts')
    .select(`
      id, title, category, city,
      post_likes(count),
      post_comments(count)
    `)
    .eq('category', 'review')
      ...
      <main className="max-w-[1200px] mx-auto px-4 py-6">
        <div className="grid grid-cols-[160px_1fr_160px] gap-6">
          ...
          <div className="flex flex-col gap-6">
            <SearchZone locale={params.locale} />
            <PlaceGrid places={popularPlaces || []} locale={params.locale} />
            <BannerAd />
            <BestReviews posts={bestPosts || []} locale={params.locale} />
            <CommunityGrid posts={latestPosts || []} locale={params.locale} />
          </div>
